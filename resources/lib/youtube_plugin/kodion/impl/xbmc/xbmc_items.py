...
def to_play_item(context, play_item):
    context.log_debug(f'Converting PlayItem |{play_item.get_uri()}|')
    ...
    thumb = play_item.get_image() or u'DefaultVideo.png'
    title = play_item.get_title() or play_item.get_name()
    fanart = ''
...
def to_video_item(context, video_item):
    context.log_debug(f'Converting VideoItem |{video_item.get_uri()}|')
    major_version = context.get_system_version().get_version()[0]
    thumb = video_item.get_image() or u'DefaultVideo.png'
    title = video_item.get_title() or video_item.get_name()
    fanart = ''
    settings = context.get_settings()
    if major_version > 17:
    ...
    if not video_item.live:
        ...
        if use_dt := scheduled_start or published_at:
...
def to_audio_item(context, audio_item):
    context.log_debug(f'Converting AudioItem |{audio_item.get_uri()}|')
    major_version = context.get_system_version().get_version()[0]
    thumb = audio_item.get_image() or u'DefaultAudio.png'
